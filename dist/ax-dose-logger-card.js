      ...
      <div class="pane pane-daily">
        <div class="daily-main">
          ...
          >
            ...
            <span class="take-sub"><span class="take-sub-segment">${Te(this._lang,"daily.last")}: ${a}</span>${n?W` \u2022 <span class="take-sub-segment">${"daily.slot_remaining"===n.key?Te(this._lang,"daily.slot_remaining",{count:n.value}):Te(this._lang,n.key)}: ${n.value}</span>`:V}</span>
          ...
          <div class="stats-column">
            <div class="stat-pill ${E?"clickable":""}"
                 role="button"
                 tabindex=${E?0:-1}
                 aria-label=${e.config?.safe_to_take_label||$}
                 @click=${E?He(t=>e.handleSafeBoxAction(t,"tap",A,y)):null}
                 @keydown=${E?t=>e.onKeyActivate(t,()=>e.handleSafeBoxAction(null,"tap",A,y)):null}
                 @contextmenu=${S?t=>{t.preventDefault(),e.handleSafeBoxAction(null,"hold",A,y)}:null}
                 @dblclick=${I?()=>e.handleSafeBoxAction(null,"double_tap",A,y):null}>
              ${E?W`<ha-ripple></ha-ripple>`:V}
               <ha-icon icon="${e.config?.safe_to_take_icon||D}"></ha-icon>
              <span class="stat-label">${e.config?.safe_to_take_label||$}</span>
              <span class="stat-value">${x?Te(this._lang,"daily.na"):v&&!w?(()=>{const i=parseFloat(k),a=e.getStrengthUnit(t);return isNaN(i)?k:`${Math.round(i)}${a?" "+a:""}`})():w?k?isNaN(parseFloat(k))?k.charAt(0).toUpperCase()+k.slice(1):e.formatInteger(k)+(e.getAttr(y,"unit_of_measurement")?" "+e.getAttr(y,"unit_of_measurement"):""):"":e.formatInteger(i)}</span>
            </div>
            <div class="stat-pill ${u?"clickable":""}"
                 role="button"
                 tabindex=${u?0:-1}
                 aria-label=${e.config?.pills_left_label||m}
                 @click=${u?He(t=>e.handlePillsLeftBoxAction(t,"tap",h,s,f)):null}
                 @keydown=${u?t=>e.onKeyActivate(t,()=>e.handlePillsLeftBoxAction(null,"tap",h,s,f)):null}
                 @contextmenu=${g?t=>{t.preventDefault(),e.handlePillsLeftBoxAction(null,"hold",h,s)}:null}
                 @dblclick=${_?()=>e.handlePillsLeftBoxAction(null,"double_tap",h,s):null}>
              ${u?W`<ha-ripple></ha-ripple>`:V}
               <ha-icon icon="${e.config?.pills_left_icon||b}"></ha-icon>
              <span class="stat-label">${e.config?.pills_left_label||m}</span>
              <span class="stat-value">${c?Te(this._lang,"daily.na"):r?e.formatInteger(l):d?l?isNaN(parseFloat(l))?l.charAt(0).toUpperCase()+l.slice(1):e.formatInteger(l)+(e.getAttr(s,"unit_of_measurement")?" "+e.getAttr(s,"unit_of_measurement"):""):"":"unavailable"===l?"-":e.formatInteger(l)}</span>
            </div>
          </div>
        </div>
