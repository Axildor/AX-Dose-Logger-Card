...
## HACS Foundational File Architecture (2026-08-11)

**Feature:** Generated the foundational HACS-compliance file set for the frontend card repository so it meets the HACS default-repository inclusion standards for a **plugin** category repository. Three artefacts: a new GitHub Actions HACS validation workflow, the required `render_readme: true` key in `hacs.json`, and confirmation that the existing README already satisfies HACS documentation requirements. No backend changes (plugin-only repo; no Hassfest/Ruff). Architecture plan: [`plans/hacs-foundational-architecture-plan.md`](plans/hacs-foundational-architecture-plan.md).

### Checklist
- [x] Read frontend memory-bank (activeContext, progress, projectstructure) + frontend repo files (hacs.json, package.json, rollup.config.js, README.md, .github/) for context
- [x] Confirm compiled JS output filename = `ax-dose-logger-card.js` (from [`rollup.config.js:7`](rollup.config.js) + [`package.json:5`](package.json))
- [x] Confirm `.github/workflows/` does not exist; README + hacs.json present
- [x] Author plan doc embedding raw code for all 3 files → [`plans/hacs-foundational-architecture-plan.md`](plans/hacs-foundational-architecture-plan.md)
- [x] Verify README.md already HACS-compliant (overview lines 1–17; HACS install lines 56–64; manual install lines 66–71; 60-row config variables table lines 257–318) — no change needed
- [x] Create [`.github/workflows/hacs.yaml`](.github/workflows/hacs.yaml) — `name: HACS Validation`; `on: push/pull_request` to `main`; single `hacs` job on `ubuntu-latest`; steps: `actions/checkout@v4` + `hacs/action@main` with `category: plugin`
- [x] Update [`hacs.json`](hacs.json) — insert `"render_readme": true` as the second key (between `name` and `filename`); other keys unchanged
- [x] Validate JSON syntax: `python3 -m json.tool hacs.json` → VALID
- [x] Validate YAML syntax: `python3 -c "import yaml; yaml.safe_load(open('.github/workflows/hacs.yaml'))"` → VALID
- [x] Confirm `render_readme` present (grep → line 3) + `category: plugin` present (grep → line 18)
- [x] Update [`memory-bank/activeContext.md`](memory-bank/activeContext.md) (new Current Status; prior archived)
- [x] Update [`memory-bank/projectstructure.md`](memory-bank/projectstructure.md) (new `.github/workflows/hacs.yaml` in tree + File Responsibilities table; `hacs.json` role updated to mention `render_readme`)
- [x] Update [`memory-bank/progress.md`](memory-bank/progress.md) (this section)

### Key decisions
1. **`category: plugin` (not `integration`)** — the repo is a Lovelace custom card (frontend), so the HACS action must validate against the plugin schema. This is the single mandatory argument for a card repository's HACS workflow.
...
3. **`actions/checkout@v4`** — v3 is deprecated since 2024-Q4; v4 is the current stable checkout action. No `permissions:` block needed — `hacs/action` runs read-only validation.
4. **`render_readme: true`** — required by the task; tells HACS to render the repo README on the HACS store page. Placed as the second key (governs store presentation).
5. **README left untouched** — the existing 340-line production README already exceeds the HACS documentation bar (overview + HACS/manual install + 60-row config table). Replacing it with a stripped template would regress the repository; the user explicitly approved keeping it.
6. **No backend workflows** — strictly a plugin repository; Hassfest/Ruff belong in the backend repo, not here.
7. **No `yarn run build` needed** — no `src/` changes; the HACS files are repo-metadata, not compiled artefacts.

### Verification
- `python3 -m json.tool hacs.json` → VALID JSON (5 keys: name, render_readme, filename, homeassistant, hacs)
