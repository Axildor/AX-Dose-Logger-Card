...
## Archived from progress.md on 2026-08-08 (truncation — kept last 16 sections; oldest 11 moved here)

## Current Label Overlap Fix — Chips Left / Current Right (2026-08-08)

### Planning
- [x] Read frontend memory-bank/activeContext.md + projectstructure.md for context
- [x] Read graphs-panel.ts (_renderLineGraph, _renderBarGraph, _renderEffectivenessGraph, CSS) — confirmed root cause: SVG "Current:" `<text>` at x=padLeft (polyline start) + y≈currentY (dashed line) → polyline crosses through label, SVG text has no background
- [x] Confirm fix direction with user: move timeframe chips to LEFT, render current value as a matching HTML pill chip on the RIGHT
- [x] Write architecture plan to plans/current-label-overlap-fix-plan.md (6-step implementation, key design decisions, verification)

### Implementation
- [x] localize.ts: add `graphs.current` = 'Current' key (replaces old hardcoded SVG `Current:` text — small localization win)
- [x] graphs-panel.ts CSS: `.timeframe-chips` `right: 4px` → `left: 4px` (one rule, all 3 graphs share the class)
- [x] graphs-panel.ts CSS: new `.current-label` class — top-right pill mirroring `.timeframe-chip` styling (same font-size 12px, padding 4px 10px, border-radius 4px, bg opacity 0.08) but uses --primary-color text so the value pops
- [x] graphs-panel.ts SVG geometry: padTop 28→36 + h +8 on bar (180→188), line (200→208), effectiveness (196→204) so chartH stays 144 (matched-Y-axis invariant) AND left chips clear the top Y-axis label/gridline
- [x] graphs-panel.ts: remove `currentLabelY` computation (only used by deleted SVG text); rename gate to `hasCurrent` for reuse
- [x] graphs-panel.ts: remove SVG "Current:" `<text>` block (keep the dashed `<line>` as visual anchor); update comment
- [x] graphs-panel.ts: add HTML `.current-label` chip in `.line-graph-wrapper` template after timeframe-chips div, gated on `hasCurrent`
- [x] graphs-panel.ts: update header comments on bar/line/effectiveness graphs documenting the new h/padTop rationale

### Verification
- [x] yarn run build — clean (exit 0, no warnings, dist/ax-dose-logger-card.js created in 3.6s)
- [x] Dist grep confirms: `graphs.current` key (line 343), `.current-label` HTML chip with `hasCurrent` gate (lines 3424-3427), `.timeframe-chips` now `left: 4px` (line 3884), `.current-label` CSS `right: 4px` (lines 3897-3901); no stray old `Current: ` SVG text
- [x] No backend / coordinator / store / config-flow / editor / types / README changes (frontend-only bug fix)
- [x] No projectstructure.md change (no files added/renamed/deleted)

### Follow-on Refinements (user feedback, same day)
- [x] Confirm exact left-gap value with user (chip ~41px wide → "half a chip more than 4px" = `left: 24px`)
- [x] graphs-panel.ts CSS: `.timeframe-chips` `left: 4px` → `left: 24px` (~half a chip length gap from the graph window border)
- [x] graphs-panel.ts CSS: `.timeframe-chip` add `min-width: 41px` + `box-sizing: border-box` + `text-align: center` so 2-char labels ("7D") render the same width as 3-char labels ("12H"/"48H"/"14D")
- [x] graphs-panel.ts CSS: `.current-label` `color: var(--primary-color)` → `color: var(--secondary-text-color)` to match the inactive timeframe chips' color (reserved primary-color highlight for the active selected timeframe chip only)
- [x] graphs-panel.ts CSS: `.current-label` `right: 4px` → `right: 10px` (+6px right-side gap per user request, mirrors the left-side chips' offset from the graph window border)
- [x] yarn run build — clean (exit 0, no warnings, dist/ax-dose-logger-card.js created in 4s then 3.9s)
- [x] Dist grep confirms: `.timeframe-chips` `left: 24px` (line 3884), `.timeframe-chip` `min-width: 41px` + `box-sizing: border-box` + `text-align: center` (lines 3927-3929), `.current-label` `color: var(--secondary-text-color)` (line 3903), `.current-label` `right: 10px` (line 3898)
...
## Daily/Drinks Button Two-Line Height Lock (2026-08-08) [archived from root progress.md on 2026-08-09]
...
## Consecutive-Press Background Pulse Animation (2026-08-10)

**Feature:** Mushroom-Template-card-style "vibrant shade" pulse on qualifying clickable elements. On press, the element's background flashes a vibrant tint (primary blue, red for danger) over 400ms; rapid re-presses restart the animation. ACK buttons (Take Pill / Log Drink) gain a green `box-shadow` ring pulse on the 2nd+ repeat press alongside the existing `Nx` counter.

### Checklist
- [x] Step 1: Architecture plan drafted — [`plans/consecutive-press-pulse-plan.md`](plans/consecutive-press-pulse-plan.md) (scope confirmed with user: IN = ACK buttons + repeatable action buttons + dialog-opener boxes/chips; OUT = nav bar + Graphs state toggles; non-ACK buttons get pulse only, no `Nx`)
- [x] Step 2: Created shared CSS fragment — NEW [`src/styles/pulse.ts`](src/styles/pulse.ts) exporting `pulseStyles` (`ax-press-pulse` keyframe + `.ax-pulse` class, 400ms) + `PULSE_MS = 400`
- [x] Step 3: Added `pulseClick(ev, handler)` helper to [`src/helpers.ts`](src/helpers.ts) — adds `.ax-pulse` class, forces reflow for rapid re-press restart, schedules removal after `PULSE_MS`, calls original handler. Zero Lit `@state`.
- [x] Step 4: Adopted pulse on tools-panel.ts — wrapped 5 tool buttons + per-drink Undo/Reset; danger buttons override `--ax-pulse-color` to error tint; added `${pulseStyles}`
- [x] Step 5: Adopted pulse on inventory-panel.ts — wrapped refill box + avg-cell; added `${pulseStyles}`
- [x] Step 6: Adopted pulse on stats-panel.ts — wrapped stat-cell more-info openers; added `${pulseStyles}`
- [x] Step 7: Adopted pulse on daily-panel.ts — wrapped med-name, Safe-to-Take, Pills Left, chips (Take Pill left unwrapped); added `${pulseStyles}`; added `ax-ack-pulse` keyframe + second animation on `.ack-flash.ack-repeat` (green ring, 600ms)
- [x] Step 8: Adopted pulse on drinks-panel.ts — mirror of daily (drinks-title, In Body, Disruption, chips + `ax-drink-ack-pulse` keyframe)
- [x] Step 9: Adopted pulse on ax-dose-logger-card.ts — wrapped dialog action buttons (device-info, Refill Confirm, Log Drink rows, Override Confirm, Tools Confirm, Tracking Confirm); added `${pulseStyles}` to container stylesheet
- [x] Step 10: `yarn run build` — exit 0, 3.9s, no TS errors
- [x] Step 11: Dist grep — `grep -c 'ax-press-pulse\|pulseClick\|ax-ack-pulse\|ax-drink-ack-pulse\|ax-pulse' dist/ax-dose-logger-card.js` = 46
- [x] Step 12: README updated — added "Consecutive-press pulse" paragraph after the Rapid successive clicks paragraph
- [x] Step 13: Memory bank updated — activeContext.md (new status + 2 retained Previous Context, oldest archived to old/activeContext-archive.md), progress.md (this section)

### Key decisions
1. **CSS-class + reflow, not Lit `@state`** — self-contained per element via `classList.add/remove` + forced reflow. Replicating the ACK counter's reactive pattern for ~30 elements would add 60 `@state` + 30 timers to the container. The CSS approach is the Mushroom Template card pattern — zero Lit state, browser-driven, self-cleaning. No `shouldUpdate` whitelist change.
2. **ACK pulse is a `box-shadow` ring** — the `.ack-flash` overlay already paints an opaque `--btn-green-soft` background; a brighter background would hurt tick/text legibility. The ring expands outward and fades — the "vibrant shade" cue as a green halo. Driven by the existing `keyed(ackCount, …)` recreation, so the animation restarts naturally with zero extra state.
3. **No `Nx` on non-ACK buttons** (user-confirmed) — counting dialog/more-info opens is odd; the pulse is the only feedback there.
4. **Exclude state-toggle controls** (user-confirmed) — Graphs timeframe chips, carousel nav, effectiveness tabs are single-shot state switches; a rapid-repeat pulse reads oddly there.
5. **Danger buttons override `--ax-pulse-color`** to the error tint (red) via a per-element CSS var override.
6. **Unique keyframe names per Shadow DOM** — `ax-ack-pulse` (daily) vs `ax-drink-ack-pulse` (drinks) to avoid Lit CSS keyframe collisions.
7. **No new config option** — always-on, matching the ACK counter directive. No editor-schema change.

## Press Ripple — ha-ripple Migration (2026-08-10)

**Feature:** Replaced the prior custom CSS background-pulse (rejected as "a flash so fast you can miss it" + "not smooth like Mushroom") with Home Assistant's native `<ha-ripple>` web component — 1:1 parity with Lovelace Mushroom card press feedback. Every qualifying clickable element gets a smooth Material Design radiating-circle ripple from the tap point + a subtle hover tint. ACK buttons keep `Nx`; green ring dropped.

### Checklist
- [x] Step 1: Architecture plan drafted — [`plans/consecutive-press-ripple-ha-ripple-plan.md`](plans/consecutive-press-ripple-ha-ripple-plan.md) (scope confirmed: all buttons get ha-ripple; ACK keeps Nx, drops green ring; nav + Graphs toggles excluded)
- [x] Step 2: Inspected ha-ripple source from HA frontend bundle (`/usr/local/lib/python3.14/site-packages/hass_frontend/frontend_latest/11314.*.js`) — confirmed API: nested `<ha-ripple></ha-ripple>`, `disabled` prop, CSS vars `--ha-ripple-color` / `--ha-ripple-hover-opacity` (0.08) / `--ha-ripple-pressed-opacity` (0.12)
- [x] Step 3: Deleted `src/styles/pulse.ts` + removed `pulseClick` from `src/helpers.ts`
- [x] Step 4: tools-panel.ts — reverted imports + @click handlers; injected `<ha-ripple>` into 7 buttons; ripple vars on `:host`; danger → red ripple; `position: relative` on `.tool-btn`; stripped `:active` scale
- [x] Step 5: inventory-panel.ts — reverted; injected `<ha-ripple>` (refill conditional); ripple vars; `position: relative` on `.stat-pill` + `.avg-cell`
- [x] Step 6: stats-panel.ts — reverted; injected `<ha-ripple>` (conditional on entityId); ripple vars; `position: relative` on `.stat-cell`
- [x] Step 7: daily-panel.ts — reverted; injected `<ha-ripple>` into Take Pill (after glow-track) + med-name + Safe-to-Take + Pills Left + chips (conditional); removed `ax-ack-pulse` keyframe + `.ack-repeat` second animation; ripple vars + state-colored ripples (red/amber/blue/green); `position: relative` on `.med-name`/`.stat-pill`/`.chip`; stripped `:active` scale + `transform` from transition; `.take-pill-btn > ha-ripple { z-index: 1 }`
- [x] Step 8: drinks-panel.ts — mirror of daily (incl. `?disabled` on Log Drink ha-ripple; `ax-drink-ack-pulse` removed; state ripples + geometry + z-index + `:active` stripped)
- [x] Step 9: ax-dose-logger-card.ts — reverted imports + 6 dialog button @click handlers; injected `<ha-ripple>` into each (Log Drink rows with `?disabled`); ripple vars on `:host`; `position: relative; overflow: hidden` on `.dialog-btn`
- [x] Step 10: `yarn run build` — exit 0, 3.8s, no TS errors
- [x] Step 11: Dist grep — 103 `ha-ripple`, 13 `ha-ripple-color`, 6 `ha-ripple-hover-opacity`, 6 `ha-ripple-pressed-opacity`, 0 leftover `ax-pulse`/`pulseClick`/`ax-ack-pulse`/`pulseStyles`/`styles/pulse`
- [x] Step 12: README updated — replaced "Consecutive-press pulse" paragraph with "Press ripple (ha-ripple)" paragraph
- [x] Step 13: Memory bank updated — activeContext.md (new status + superseded prior + 1 retained Previous Context), progress.md (this section), projectstructure.md (removed pulse.ts entry)

### Key decisions
1. **Native `<ha-ripple>` over custom CSS** — MdRipple gives the exact Mushroom radiating-circle + hover tint; the custom flash was rejected as too fast + not smooth.
2. **Nested usage (Mushroom style)** — `<ha-ripple></ha-ripple>` as first child; MdRipple auto-detects pointer events on parent.
3. **ACK: ripple only, drop green ring** (user-confirmed) — `Nx` counter stays; ripple is the only press feedback.
4. **State-colored ripples** — Take Pill ripple matches medical state (red/amber/blue/green); Log Drink lockout → red, ACK → green; danger buttons → red.
5. **Strip `:active` scale transforms** (Gemini guidance) — MdRipple is the press feedback; scale compression is redundant.
6. **Geometry `position: relative` + `overflow: hidden`** — MdRipple requirement; added to all target elements.
7. **Hover opacity 0.04** (Gemini guidance) — subtler than ha-ripple default 0.08.
8. **Runtime dependency on HA global registry** — safe in production HA (Mushroom does the same); graceful degradation outside HA.
9. **Z-index layering** — `.take-pill-btn > ha-ripple` / `.log-drink-btn > ha-ripple` at z-1 (above glow-track z-0, below content + ACK flash z-2).

## Ripple Timing — Delayed-Action (2026-08-10)
**Feature**: Fixed the `<ha-ripple>` press feedback appearing "too fast / canceled" compared to the Mushroom template card. Root cause: interactive elements used raw `@click` which fires synchronously on pointerup; the controller immediately mutates Lit `@state` (ACK flash, `Nx` counter, frozen state) and fires a service call, triggering a synchronous Lit re-render that interrupts MdRipple's `requestAnimationFrame` animation chain — the ripple freezes mid-expansion. Fix: introduced a `delayedAction()` helper that wraps each `@click` callback in a `setTimeout(…, 110ms)`, giving the native ripple ~110 ms to render past its peak before the Lit re-render. The 110 ms delay (user-specified, shorter than HA's stock `actionHandler` ~250 ms disambiguation window) is enough to let the ripple visibly expand without making the action feel sluggish. Frontend-only. Architecture plan: [`plans/ripple-timing-actionhandler-migration-plan.md`](plans/ripple-timing-actionhandler-migration-plan.md).

### Root cause
- **Raw `@click` synchronous → Lit re-render truncates MdRipple** — `@click` fires on pointerup; the controller's `@state` mutations (ACK flash, frozen state) + service call cause Lit to synchronously re-render the parent, disrupting MdRipple's `requestAnimationFrame` ripple animation. The ripple appears "too fast" / instantly canceled.
- Mushroom's `template-card.ts` uses HA's `actionHandler` directive (NOT re-exported by `custom-card-helpers` — only `fireEvent`/`handleAction`/`hasAction` are), which introduces a ~250 ms tap/double-tap disambiguation delay before the `@action` handler runs, letting the ripple render first.

### Checklist
- [x] Step 1: Context — read frontend activeContext (Press Ripple ha-ripple Migration), inspected Mushroom `template-card.ts` actionHandler usage, confirmed `ha-ripple` is the same global component Mushroom uses
- [x] Step 2: Root-cause analysis — raw `@click` synchronous re-render interrupts MdRipple; verified `actionHandler` is NOT in `custom-card-helpers` exports
- [x] Step 3: Architecture plan — plans/ripple-timing-actionhandler-migration-plan.md (initial actionHandler plan, then revised to custom `delayedAction` helper after the export gap discovery + user's 100-125 ms delay constraint)
- [x] Step 4: User confirmation — full migration (all 15 interactive element types) approved with "delay should only be 100-125 ms" → chose 110 ms
- [x] Step 5: NEW `src/delayed-action.ts` — `delayedAction(cb)` wraps a click callback in `setTimeout(cb, 110)`; cancels pending timer on rapid re-press so the action fires once per click; exports `RIPPLE_ACTION_DELAY_MS = 110`
- [x] Step 6: `src/components/daily-panel.ts` — import + 5 handlers wrapped (med-name, take-pill-btn, safe-to-take stat-pill, pills-left stat-pill, chips); `@keydown` left immediate for a11y; `@contextmenu`/`@dblclick` left as-is (browser-disambiguated, don't interfere with ripple)
- [x] Step 7: `src/components/drinks-panel.ts` — import + 5 handlers (drinks-title, log-drink-btn, in-body, disruption, drink chips)
- [x] Step 8: `src/components/inventory-panel.ts` — import + 2 handlers (refill stat-pill, avg-cell)
- [x] Step 9: `src/components/stats-panel.ts` — import + 1 handler (stat-cell more-info)
- [x] Step 10: `src/components/tools-panel.ts` — import + 7 handlers (2 drink-tool-btns + 5 maintenance tool-btns)
- [x] Step 11: `src/ax-dose-logger-card.ts` — import + 7 ripple-bearing dialog-btn handlers (device-info navigate, color-explainer, refill confirm, log-drink picker rows, override confirm, tools confirm, tracking confirm); muted cancel/close buttons left as-is (no `<ha-ripple>`)
- [x] Step 12: Verification — `yarn run build` clean (exit 0, 4s, no TS errors); dist grep: 41 `delayedAction`/`setTimeout` occurrences, `110` delay value present 3×; logic trace: press → ripple renders ~110ms → ACK flash + service call fire → no truncation
- [x] Step 13: Update memory-bank — activeContext.md (new Current Status), progress.md (this section). No projectstructure.md change (new file `src/delayed-action.ts` added — will update projectstructure). No README change (internal timing fix, not user-facing behavior beyond "ripple now renders fully").

### Key decisions
1. **Custom `delayedAction` over `actionHandler`** — `actionHandler` is a HA frontend internal not re-exported by `custom-card-helpers`; re-implementing a directive is heavier than a `setTimeout` wrapper. The helper is 1 import + `delayedAction(cb)` per handler — minimal surface.
2. **110 ms delay** — user-specified (100-125 ms range). Enough for MdRipple to render past its peak (~300 ms full duration, but the visible expansion completes within ~120 ms) without making taps feel laggy. HA's stock actionHandler uses ~250 ms (too slow per user).
3. **`@keydown` stays immediate** — keyboard activation (Enter/Space) doesn't produce a ripple and should fire instantly for accessibility; only pointer `@click` gets the delay.
4. **`@contextmenu`/`@dblclick` untouched** — these are browser-disambiguated (hold = long-press contextmenu, double-tap = dblclick) and don't trigger a synchronous re-render before the ripple; the delay on `@click` covers the tap case.
5. **Rapid-press cancel** — `delayedAction` clears any pending timer on a second press so the action fires once per click even during rapid tapping (preserves the `Nx` rapid-click counter semantics).
6. **Muted cancel/close buttons not wrapped** — they have no `<ha-ripple>` (no press feedback to protect) and closing a dialog instantly is better UX than a 110 ms wait.
7. **No backend / config-flow / editor / types / localize / README change** — pure frontend event-timing fix.
...
## ACK Overlay Ripple Visibility (2026-08-10)
**Feature**: Fixed the green "Logged / Logged Nx" overlay showing no `ha-ripple` press feedback when the Take Pill / Log Drink button is pressed, and on rapid presses (Logged → Logged 2x → 3x). The native `<ha-ripple>` was firing correctly on every `pointerdown` but rendered *underneath* the opaque green `.ack-flash` overlay (z-index 2), which fully occluded it. Frontend-only, pure CSS fix. Architecture plan: [`plans/ack-overlay-ripple-visibility-plan.md`](plans/ack-overlay-ripple-visibility-plan.md).

### Root cause
The `.ack-flash` overlay has `z-index: 2` + an **opaque** dark-green background (`--btn-green-soft: #212c22`), while the button's native `<ha-ripple>` sat at `z-index: 1` — the opaque green surface painted *on top of* the ripple, fully occluding it once the ACK flash mounted (~110 ms after the press via `delayedAction`). On rapid presses, `keyed(ackCount)` recreated the overlay each time, and each fresh opaque surface again covered the ripple. The `<ha-ripple>` fires at `pointerdown` (t=0, MdRipple supports concurrent waves) — it just rendered where nobody could see it.

### Checklist
- [x] Step 1: Context grounding — read frontend memory-bank activeContext (Ripple Timing — Delayed-Action), [`src/delayed-action.ts`](src/delayed-action.ts), daily-panel.ts + drinks-panel.ts CSS + templates; located `.ack-flash` z-index:2 + opaque `--btn-green-soft` background vs `<ha-ripple>` z-index:1
- [x] Step 2: Architecture plan — [`plans/ack-overlay-ripple-visibility-plan.md`](plans/ack-overlay-ripple-visibility-plan.md) (root cause = opaque overlay above ripple, pure-CSS z-index 3 + white tint fix, layering table, out-of-scope)
- [x] Step 3: User confirmation — plan approved ("Plan looks good — switch to code mode and apply the CSS edits")
- [x] Step 4: Apply CSS edits — [`src/components/daily-panel.ts`](src/components/daily-panel.ts): `.take-pill-btn > ha-ripple` z-index 1→3, `.take-pill-btn.ack-flash --ha-ripple-color` `var(--btn-green)`→`#ffffff`, comment blocks updated; [`src/components/drinks-panel.ts`](src/components/drinks-panel.ts): mirror edits
- [x] Step 5: Verification — `yarn run build` clean (exit 0, 4s, no TS errors); dist grep: 4 occurrences of new CSS (`z-index:3` ×2, `#ffffff` ×2)
- [x] Step 6: Memory bank — activeContext updated (new Current Status, prior Ripple Timing demoted to Previous Context); progress section added

### Notes
- No JS / template changes — the native ripple already fires correctly; the fix is purely stacking-context + colour.
- `delayedAction` 110 ms timing unchanged (orthogonal to this fix; both work together: ripple fires t=0, renders fully ~110 ms, stays visible over green overlay appearing ~110 ms).
- Non-ACK button states unaffected (`.ack-flash` absent → ripple renders over normal surface, same visual).
- Matches Mushroom template-card layering convention (ripple renders over content).
...
--- Archived from root progress.md on 2026-08-11 (Z-Axis Dependency Fix truncation) ---
...
## Logged Dose Indicator — Clarity, Softening, Press-Feel (2026-08-09) [archived from root progress.md on 2026-08-11]
...
--- Archived from root progress.md on 2026-08-11 (HACS Foundational File Architecture truncation pass) ---
